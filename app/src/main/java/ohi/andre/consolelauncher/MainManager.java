...
// Import BuildConfig explicitly to avoid compilation errors
...
public class MainManager {
    ...
    // Regex patterns
    private final Pattern colorExtractor = Pattern.compile("(#[^(]{6})\\[([^\\)]*)\\]", Pattern.CASE_INSENSITIVE);
    private final Pattern pa = Pattern.compile("%a", Pattern.CASE_INSENSITIVE | Pattern.LITERAL);
    private final Pattern pp = Pattern.compile("%p", Pattern.CASE_INSENSITIVE | Pattern.LITERAL);
    private final Pattern pl = Pattern.compile("%l", Pattern.CASE_INSENSITIVE | Pattern.LITERAL);
    ...
    private void updateServices(String cmd, boolean wasMusicService) {
        if (keeperServiceRunning) {
            ...
            if (mainPack.currentDirectory != null) {
    ...
    public void dispose() {
        if (mainPack != null) mainPack.dispose();
    }

    public void destroy() {
        if (mainPack != null) mainPack.destroy();
        TuiLocationManager.disposeStatic();

        if (messagesManager != null) messagesManager.onDestroy();

        if (themeManager != null) themeManager.dispose();
        if (htmlExtractManager != null) htmlExtractManager.dispose(mContext);
        if (aliasManager != null) aliasManager.dispose();
    ...
    public boolean performLaunch(MainPack mainPack, AppsManager.LaunchInfo i, String input) {
        ...
        try {
            ...
            Tuils.sendOutput(mContext, "Error launching app: " + e.getMessage());
        }
